            ...
            <div class="card-body">
              <div class="form-group">
                <div v-for="metaOrigen in metaOrigen" v-bind:key="metaOrigen.meta">
                  ...
                  >
                    <option value selected disabled>Conrversión</option>
                    <option
  ...
  methods: {
    ...
    insert() {
      ...
      for (let inde = 0; inde < insertText.columns.length; inde++) {
        if (insertText.columns[inde].columnName == cambiar) {
          var guardar = insertText.columns[inde].value.toString();
          //guardar.replace(".",",");
          if (action == "Decimal (, / .)") {
            insertText.columns[inde].value = guardar.replace(",",".");;
          }
            if (action == "Decimal (. / ,)") {
            insertText.columns[inde].value = guardar.replace(".",",");;
          }
